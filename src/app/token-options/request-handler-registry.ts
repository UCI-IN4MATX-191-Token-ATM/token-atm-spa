...
export class RequestHandlerRegistry {
    ...
    public static registerRequestHandler<T extends TokenOption, R extends TokenATMRequest<T>>(
        handler: Type<RequestHandler<T, R>>
    ) {
        REGISTERED_REQUEST_HANDLERS.push(handler);
    }
}
